...
export function CopyableInline({ command }: CopyableInlineProps) {
  ...
  return (
    <button
      onClick={handleCopy}
      className="group inline-flex items-center gap-3 w-full sm:w-auto rounded-full border border-border/30 bg-zinc-950 px-5 py-3.5 font-mono text-sm transition-all hover:border-border/50"
    >
      <span className="text-zinc-500 select-none">$</span>
      <span className="text-emerald-400 flex-1 text-left">{command}</span>
      {copied ? (
...
export function CopyableBlock({ commands, title = 'Terminal' }: CopyableBlockProps) {
  ...
  return (
    <div className="rounded-2xl border border-border/30 bg-zinc-950 overflow-hidden">
      ...
      <div className="px-4 py-3.5 font-mono text-sm leading-7 space-y-2">
        {commands.map((cmd, i) => (
